...
import { info, mappingInfo, industryInfo, stdInfo, addMapping, delMapping } from '../../services/atlas/stdCategoryMapping'
...
const columns = [
  { label: '标准编号', prop: 'standardNo', align: 'center', sortable: true },
  { label: '标准名称', prop: 'standardCname' },
  { label: '标准状态', prop: 'stdSta' },
]
...
const handleDelete = async (row: any) => {
  await ElMessageBox.confirm(`确定要删除吗？`, '提示', {
      ...
      await delMapping(row.standardId, searchForm.value.categoryId)
...
const tree = async () => {
  ...
  productTree.value = treeData.data
  if(productTree.value.length > 0){
    searchForm.value.categoryId = productTree.value[0].categoryId
    onSearch()
  }
} 

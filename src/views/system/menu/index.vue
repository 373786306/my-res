      ...
      <div class="p-4 rounded shadow h-full" >
        ...
        <hr class="my-2 border-t border-gray-300">
          ...
          <div class="mt-5 mb-4 flex justify-between items-center gap-4 w-full">
            <el-button type="primary" plain icon="Plus" @click="onAdd" v-hasPermi="['system:menu:add']">新增</el-button>
          ...
          >
            ...
            <template #action="{ row }">
              ...
              <el-button size="small" text type="primary" @click="handleEdit(row)" v-hasPermi="['system:menu:edit']">修改</el-button>
              <el-button size="small" text type="danger" @click="handleDelete(row)" v-hasPermi="['system:menu:remove']">删除</el-button>
              </div>
            </template>
          </CommonTable> 
      </div>
...
import type { menu } from '../../../type/system'
import { menuInfo, addMenu, editMenu, deleteMenu } from '../../../services/system/menuService'
...
const searchForm = ref<menuSearch>({
  menuName: '',
  status: null,
})
...
async function onSearch() {
  const res = await menuInfo(searchForm.value);
  menus.value = res
}
...
const handleDelete = async (row: any) => {
  ...
  await ElMessageBox.confirm(`确定要删除：${row.menuName}？`, '提示', {
